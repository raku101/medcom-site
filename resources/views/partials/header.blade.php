...
<!-- الشريط العلوي -->
<div class="bg-[#1782a4] text-white text-sm px-4 py-2 {{ app()->getLocale() === 'ar' ? 'md:pr-[280px]' : 'md:pl-[280px]' }}">
  <div class="flex flex-col gap-y-2 md:flex-row items-center md:justify-between w-full {{ app()->getLocale() === 'ar' ? 'md:flex-row-reverse' : 'md:flex-row' }}">

    <!-- عنصر وهمي لحجز مساحة الشعار -->
   

    <!-- القسم الأول: البحث + اللغة + الوضع -->
    <!-- القسم الأول: البحث + اللغة + الوضع -->
<div class="flex items-center gap-3 mt-2 md:mt-0 relative {{ app()->getLocale() === 'ar' ? 'md:right-[-250px]' : 'md:left-[-250px]' }}">
  ...
  <!-- شعار الكمبيوتر -->
  <div class="absolute top-[0px] right-[46px] hidden md:block bg-white px-4 py-2 rounded-b-xl shadow-lg z-[60]">
    <img src="{{ asset('images/medcom-1.png') }}" alt="Logo" class="h-[120px] object-contain animate-loop-fade">
  </div>

  <!-- شريط التنقل للموبايل -->
  <div class="md:hidden flex justify-between items-center px-4 h-14 bg-white dark:bg-slate-900 shadow-md z-[100] relative">
  ...
  <nav class="hidden md:flex h-14 gap-6 font-bold text-[#333] dark:text-white text-[15px] items-center w-full {{ app()->getLocale() === 'ar' ? 'justify-start pr-[200px]' : 'justify-end pl-[200px]' }}">
   ...
   <a href="{{ route('articles.index') }}" class="hover:text-[#1782a4]">الأخبار والمقالات</a>
